...
class CustomWireMock : IntegrationTestConfiguration {
    ...
    override fun configure(applicationContext: ConfigurableApplicationContext) {
        ...
        val httpClientUrls = mapOf("client.branasstorage.product.host" to "").map {
            "${it.key}=http://localhost:${wmServer.port()}${it.value}"
        }
